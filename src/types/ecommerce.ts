...
export class List<T extends Entity> extends Array<T> {
  ...
  async fetchAll(url: string): Promise<T[] | Error> {
    ...
    if ("message" in data)
      throw new Error(`Unsuccesfull: ${jsondata.statusText}`);
    this.push(...data);
    return data;
  }
  ...
  push(...items: T[]): number {
    function existence(arr: T[], id: number) {
      return arr.some((arr2) => id === arr2.id);
    }

    if (items.map((newItem) => existence(this, newItem.id)).includes(true)) {
      return 0;
    } else {
